...
export const initMenu = (startGame: () => void) => {
  ...
  // This has to be a sibling element, behind gameoverScreen, not a child of it,
  // so that backdrop-filter can transition properly
  ...
  if (storedScore) {
    menuText1.innerText = `Highscore: ${storedScore}`;
  }
...
export const showMenu = (focus: FocusObject, firstTime: boolean) => {
  ...
  // First time game is loaded, menu background needs to be fast
  if (firstTime) {
  ...
  const storedScore = localStorage.getItem('Tiny Yurts');
  menuText1.innerHTML = storedScore
