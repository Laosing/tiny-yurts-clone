...
const restartButton = createElement('button') as HTMLButtonElement;
const menuButtonWrapper = createElement();
const menuButton = createElement('button') as HTMLButtonElement;
...
export const toggleGameoverlayButton = createElement('button') as HTMLButtonElement;

export const initGameover = (startNewGame: () => void, gameoverToMenu: () => void, toggleGameoverlay: () => void) => {
  ...
  // 24px margin-top counteracts underline in gameoverText2
...
export const showGameover = () => {
  ...
  const storedScore = localStorage.getItem('Tiny Yurts');
  if (storedScore && score > Number.parseInt(storedScore, 10)) {
    localStorage.setItem('Tiny Yurts', score.toString());
  } else if (!storedScore) {
    localStorage.setItem('Tiny Yurts', score.toString());
  }
  ...
  setTimeout(() => {
    toggleGameoverlayButton.style.pointerEvents = ''; // Is separate from gameoverWrapper
