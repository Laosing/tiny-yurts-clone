...
export class Pond {
  ...
  y: number;
  points: Point[];
  avoidancePoints: Point[];
}

const createPondShape = (width: number, height: number): Point[] => {
  const points: Point[] = [];
...
export const spawnPond = ({ width, height, x, y }: { width: number; height: number; x: number; y: number }): void => {
  let points = createPondShape(width, height);
  const avoidancePoints: Point[] = [];
