...
export const addGridToSvg = (): void => {
  // The entire game's grid, including non-buildable area off the board
  ...
  gridRect.style.transition = 'opacity 0.3s';
  ...
  gridRectRed.style.transition = 'opacity 0.3s';

  svgElement.append(gridRect, gridRectRed);
};

export const gridToSvgCoords = (object: { x: number; y: number }): { x: number; y: number } => ({
