...
import type { FarmProperties, PathPoint, PathRoute } from './types';
...
export interface FarmPropertiesWithExtras extends FarmProperties {
  delay?: number;
  fenceColor: string;
  needyness: number;
}
...
export class Farm extends GameObjectClass {
  ...
  prevNumOverflowIssues?: number;

  constructor(properties: FarmPropertiesWithExtras) {
    ...
    this.fenceColor = properties.fenceColor;
    this.needyness = properties.needyness;
  ...
  assignWarn(): void {
    const adults = this.children.filter((c) => !(c as Animal).isBaby);
    const notWarnedAnimals = adults.filter((c) => !(c as Animal).hasWarn);
    const warnedAnimals = adults.filter((c) => (c as Animal).hasWarn);

    if (this.hasWarn) {
      if (this.numIssues <= adults.length) {
        this.hideWarn();
      } else {
        this.children.forEach((c) => (c as Animal).hideWarn());
      }
    } else {
  ...
  update(gameStarted: boolean, updateCount: number): void {
    ...
    this.children.forEach((animal) => (animal as Animal).update(gameStarted));
    ...
    for (let i = 0; i < this.numIssues; i++) {
      ...
      let bestRoute: PathRoute[] | null = null;
      ...
      for (let j = 0; j < atHomePeopleOfSameType.length; j++) {
        ...
        // If this person's route has fewer nodes, it's probably faster.
        if (thisRoute && thisRoute.length < bestRoute!.length) {
          bestRoute = thisRoute;
          closestPerson = atHomePeopleOfSameType[j];
        }

        // If this person's route has the same number of nodes, but fewer diagonals?
  ...
  render(): void {
    this.children.forEach((animal) => (animal as Animal).render());
  }
  ...
  addToSvg(): void {
    ...
    if (this.type !== colors.fish) {
      ...
      gridBlock.style.transition = 'opacity 0.8s';
    ...
    shadow.style.transition = `stroke-dashoffset 1s, transform 0.5s`;
  ...
  showWarn(): void {
    ...
    if (this.pinSvg) {
      this.pinSvg.style.opacity = '1';
      this.pinSvg.style.transform = `translate(${(this.pinSvg as unknown as { translate: string }).translate}) scale(1)`;
      this.pinSvg.style.transition = `all 0.8s cubic-bezier(0.5,2,0.5,1)`;
    }
    if (this.warnCircle) {
      this.warnCircle.style.transition = 'stroke-dashoffset 0.4s 0.8s';
      setTimeout(() => {
        this.warnCircle!.style.transition = 'stroke-dashoffset 0.4s';
      }, 1000);
    }
  ...
  hideWarn(): void {
    ...
    if (this.pinSvg) {
      this.pinSvg.style.opacity = '0';
      this.pinSvg.style.transform = `translate(${(this.pinSvg as unknown as { translate: string }).translate}) scale(0)`;
      this.pinSvg.style.transition = `all 0.8s cubic-bezier(0.5, 2, 0.5, 1) 0.4s`;
    }
    if (this.warnCircle) {
      this.warnCircle.style.transition = `stroke-dashoffset 0.3s`;
    }
  }
