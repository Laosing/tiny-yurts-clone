...
interface GetRandomPositionOptions {
  width?: number;
  height?: number;
  anchor?: Anchor;
  minDistance?: number;
...
const getRandomNewType = () => {
  ...
  return goodRatioTypes.at(Math.floor(Math.random() * goodRatioTypes.length));
};
...
const getRandomExistingType = () => {
  ...
  const yurtTypeCounts: Record<string, number> = {};
  ...
  const farmTypeCounts: Record<string, number> = {};
...
export const getRandomPosition = ({
  ...
  while (numAttempts < maxNumAttempts) {
    ...
    const pointOverlapsWithExtra = (point: { x: number; y: number }) => point.x === x + extra.x && point.y === y + extra.y;
    ...
    // previously, beause yurts always have paths under them and path was detected
    const yurtObstruction = yurts.some((yurt) => {
      for (let w = 0; w < width; w++) {
        for (let h = 0; h < height; h++) {
...
const getRandomYurtProps = () => {
  // Which way is yurt facing (randomly up/down/left/right to start)
...
export const spawnNewObjects = (updateCount: number, delay?: number) => {
  ...
  // Spawn the first farm, early on, near center
  if (
    (updateCount === 0)
    || (
      updateCount > 1000
    ...
    if (updateCount > 10000 && !fishFarms.length) {
      ...
      const randomPosition = getRandomPosition({
        width,
        height,
        anchor: farms.length > 1 // So for 3rd farm...
          ? yurts.filter((y) => y.type === type).at(Math.floor(Math.random() * yurts.length))
          : farms[farms.length - 1], // if undefined randomPosition will use default
        maxDistance: farms.length + 2,
        minDistance: farms.length ? 2 : 0,
        // We _need_ this to work on 1st loop otherwise menu breaks.
  ...
  // Spawn first yurt really soon after
  if (updateCount % spawningLoopLength === 500 + (farms.length > 1 ? updateRandomness2 : 0)) {
    ...
    const farm = (
      ...
      : farms.length > 2
        ? farms.at(Math.floor(Math.random() * farms.length))
  ...
  // If first yurt spawn attempt failed, try again ~400ms later
  ...
  // If first yurt re-spawn attempt failed, try AGAIN ~400ms later
  ...
  if (updateCount % spawningLoopLength === 1500 + updateRandomness3) {
    ...
    const friendYurt = sameTypeYurts.at(Math.floor(Math.random() * sameTypeYurts.length));
    if (!friendYurt) return;
  ...
  // Extra farm/upgrade attempt every update, not long before per-round farm/upgrade
