...
let dragStartCell: Cell = {};
let isDragging = false;

const yurtInCell = (x: number, y: number) => yurts.find((yurt) => yurt.x === x && yurt.y === y);
const pondInCell = (x: number, y: number) => ponds.find((pond) => pond.points.find((p) => p.x === x && p.y === y));
const pondPathInCell = (x: number, y: number) => paths
  .find((path) => path.points[1].x === x && path.points[1].y === y && path.points[1].stone);

const samePathInBothCell = (x0: number, y0: number, x1: number, y1: number) => paths.find((path) => (
  (
...
const handleHazardPointermove = (event: PointerEvent) => {
...
const handlePointermove = (event: PointerEvent) => {
  ...
  const dragStartSvgPx = new Vector(
    toSvgCoord(dragStartCell.x!),
    toSvgCoord(dragStartCell.y!),
  );
  ...
  if (inventory.paths <= 0) {
    ...
    setTimeout(() => {
      ...
      pathTilesIndicatorCount.innerText = inventory.paths.toString();
    }, 300);
