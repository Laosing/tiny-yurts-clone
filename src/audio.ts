...
// main menu button? But we don't want to redo it ever as well, hmm
...
export const playSound = (
  ...
  lowpassFrequency: number = 10000,
  // Most sound above this frequency (Hz) goes through
  highpassFrequency: number = 100,
  noise: () => number = () => (2 * Math.random() - 1),
): void => {
  if (!soundSettings.on) return;
...
const warnNotes: Record<string, { currentIndex: number; notes: AudioNote[] }> = {
  [colors.ox]: {
    currentIndex: 0,
    notes: [
...
export const playPathPlacementNote = (): void => {
  if (audioContext) {
    // frequencyIndex, noteLength, playbackRate, pingyness, volume, lowpass, highpass
...
export const playPathDeleteNote = (): void => {
  if (audioContext) {
    // frequencyIndex, noteLength, playbackRate, pingyness, volume, lowpass, highpass
...
export const playTreeDeleteNote = (): void => {
  if (audioContext) {
    playSound(10, 0.1, 1, 1000, 0.2, 1500, 500, () => 2);
  }
};

export const playYurtSpawnNote = (): void => {
  if (audioContext) {
    playSound(39, 0.1, 0.25, 10, 0.2, 1000, 100); // E# (i.e. F. Music is weird)
  }
};

export const playOutOfPathsNote = (): void => {
  if (audioContext) {
    // frequencyIndex, noteLength, playbackRate, pingyness, volume, lowpass, highpass
...
export const playWarnNote = (animalType: string): void => {
  if (audioContext) {
    ...
    playSound(noteInfo[0], noteInfo[1], noteInfo[2], noteInfo[3], noteInfo[4], noteInfo[5], noteInfo[6]);
