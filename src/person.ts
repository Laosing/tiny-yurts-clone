...
import type { PathRoute } from './pathfinding';
...
export interface PersonProperties {
  x: number;
  y: number;
  parent: { type: string; x: number; y: number };
}

export interface Person extends GameObjectClass {
  type: string;
  atHome: boolean; // Is this person sitting in their yurt?
  atFarm: number; // Is the person at a farm? How long have they been there?
  destination: { x: number; y: number } | null;
  goingHome?: boolean;
  hasDestination: boolean;
  route: PathRoute[];
  originalRoute: PathRoute[];
  farmToVisit: Farm;
  svgElement?: SVGPathElement;
  parent?: { type: string; x: number; y: number }; // Parent entity (yurt)
  shadowElement?: SVGPathElement;
}
...
export class Person extends GameObjectClass {
  ...
  constructor(properties: PersonProperties) {
    ...
    this.type = properties.parent.type;
    ...
    this.route = [];
    this.originalRoute = [];
    this.farmToVisit = {} as Farm;

    // Parent x/y is in grid coords instead of SVG coords, so need to convert
    this.x = gridCellSize / 2 + properties.parent.x * gridCellSize + xVariance;
    this.y = gridCellSize / 2 + properties.parent.y * gridCellSize + yVariance;
  ...
  addToSvg(): void {
    ...
    shadow.setAttribute('d', 'M0 0 0.3 0.3');
  ...
  update(): void {
    ...
    if (this.atFarm) {
      ...
      // originalRoute.length counts every cell including yurt & farm
      if (
    ...
    // TODO: We have 3 variables for kinda of the same thing but maybe we need them
    if (this.hasDestination) {
      if (this.destination) {
        if (this.route?.length) {
          ...
          // If a yurt and farm are adjacent, you don't need to rush...
    ...
    if (this.route?.length > 0) {
      ...
      potentialCollisionPeople.forEach((otherPerson) => {
        ...
        if (nextDistanceBetween < slowyDistance && this.velocity.size > 0.06) {
